...
않고 `pointerdown`/`pointermove`/`pointerup`, 5px 시작 임계값과 pointer capture로 Dock
순서 변경과 파일 이동을 처리한다. 후보 또는 활성 drag 동안에는 위젯의 커서 이벤트
투과를 해제하고 종료 직후 마지막 전역 커서 위치로 투과 상태를 복구한다.
...
Wallpaper Engine                    --dev-window
 -parentHWND                            │
       │                                │
       ▼                                ▼
HwndSource child                  WPF debug Window
       └──────────────┬─────────────────┘
                      ▼
┌──────────────────────────────────────────────┐
│ shared WallpaperView                         │
...
첫 3D renderer는 `WebView2CompositionControl` 안의 three.js `WebGLRenderer`다. 일반
...
의존하지 않는다. composition capture 비용은 개발 창과 Wallpaper Engine의 실제
...
| `Wallpaper.Hosts` | Wallpaper Engine 실행 인자·HWND·수명주기 어댑터 |
...
| `Wallpaper.Hosts.Tests` | parent HWND 계약, pause/recovery/종료 회귀 테스트 |

구체 프로젝트 수는 첫 vertical slice에서 불필요한 추상화가 확인되면 줄일 수 있지만,
Core가 WPF 및 특정 호스트를 참조하지 않는 의존 방향은 유지한다.

`Wallpaper.Rendering.Abstractions`는 호스트가 소비하는 pause/resume 계약을 유지한다.
`Wallpaper.Rendering.WebView`가 composition surface, WebView2 수명과 scene 명령을 구현하고,
three.js scene registry에는 렌더 경로 검증용 `Baseline` 장면만 둔다. 작품별 연출과 오디오
입력 계약은 시각 방향과 요구사항을 구체화한 뒤 별도 vertical slice로 추가한다.
...
WPF 프로세스가 제품의 상태와 파일 기능을 소유하고 Wallpaper Engine을 유일한 제품
호스트로 사용한다. Wallpaper Engine은 다음 정보만 전달하거나 제어한다.
...
Wallpaper Engine 설정 형식, IPC와 HWND 검증은 어댑터 내부에 둔다. ViewModel, 파일
시스템 서비스와 Core 모델은 호스트의 존재를 알지 않는다. 호스트 없는 로컬 검증은
`--dev-window`가 같은 `WallpaperView`를 일반 WPF 창에 담아 제공한다. 이는 독립 제품
모드가 아니라 디버그 프레젠테이션이다.

루트 설정은 WPF 애플리케이션이 소유하고 앱 로컬 설정에 저장한다. 변경은 WPF 내부
설정 패널과 네이티브 폴더 선택기로만 수행한다. Wallpaper Engine 속성, 시스템 트레이
또는 호스트 내부 파일은 설정 경로로 사용하지 않는다.
...
`-WINDOWED -parentHWND <decimal HWND>`다. WPF 앱은 전달된 parent를
`HwndSourceParameters.ParentWindow`로 지정해 `WS_CHILD` HWND를 처음부터 만들고 그
`RootVisual`에 공용 `WallpaperView`를 넣는다. adapter는 parent 관계를 검증하고 parent
client rect 크기에 맞추며 다음 Desktop 계층의 유효성을 관리한다. 제품 실행에서
`-parentHWND`가 없으면 즉시 실패하며 일반 창으로 자동 폴백하지 않는다.
...
설치하지 않는다. Wallpaper Engine parent 아래에는 WPF HWND 하나만 두고 별도 UI overlay
HWND를 만들지 않는다. `WebView2CompositionControl`은 이 WPF visual tree 안에 유지하며
내부 `Chrome_WidgetWin_0`을 열거하거나 부모 변경하지 않는다. 따라서 WebView 배경과 폴더
UI의 z-order·hit testing을 WPF가 계속 소유한다. WorkerW는 여러 모니터의 wallpaper child가
공유하므로 window region으로 자르지 않는다. dispose와 종료 watchdog은 다른 활성
wallpaper child가 없을 때만 WorkerW 입력 상태와 Desktop z-order를 복원한다.
...
[ADR 0010](decisions/0010-wallpaper-engine-application-host.md)과
[ADR 0012](decisions/0012-wallpaper-engine-first-presentation.md), 실제 검증은
...
drag 상태 머신이다. `WallpaperView` 전체에서 포인터 이동과 해제를 추적하고 Dock 카드를 직접
...
- Wallpaper Engine 프로세스 생명주기 검증
- 임시 루트 파일 이동과 watcher 통합 검증
- Glass/Blur 렌더링 및 성능 확인

## 7. 검증 순서

1. `--dev-window`에서 고정 snapshot 렌더링
2. Windows 임시 루트 read-only snapshot
3. watcher와 전체 재스캔
4. 임시 루트 내부 파일 이동
5. 커스텀 우클릭과 입력 소유권
6. Wallpaper Engine 로컬 Application Wallpaper의 parent-first HWND와 입력 검증
7. 사용자가 명시적으로 선택한 실제 루트
