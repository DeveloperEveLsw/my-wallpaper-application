...
- 실제 WPF HWND owner와 화면 좌표를 사용하는 `TrackPopupMenuEx`
- `IContextMenu2/3` owner-drawn 메뉴 메시지 전달과 선택 명령 `InvokeCommand`
...
- Core 테스트 42개 통과
- Windows Infrastructure 테스트 26개 통과
...
| 파일 추가 옵션 | 설치된 shell extension과 하위 메뉴를 포함한 실제 메뉴가 표시됨 |
| 폴더 추가 옵션 | 폴더 전용 실제 메뉴가 표시됨 |
| Shell 명령 실행 | 폴더 `속성` 명령을 선택해 실제 Windows 속성창이 열림 |
| 배경 우클릭 | `디스플레이 설정`, `개인 설정`을 포함한 실제 Desktop 메뉴가 표시됨 |
...
| 네이티브 메뉴 취소 | `Esc` 뒤 WPF 포커스와 열린 파일 모달이 정상 복귀함 |
...
- `Windows 추가 옵션 표시`를 선택해 파일과 폴더의 실제 메뉴가 열리는지 확인한다.
