...
- Windows Seelen 실기기 검수: Blob 수정본 통과, 잔여 매트릭스 대기
- M1 판정: **보류**
- 기준일: 2026-07-24
...
손상된 시험 PNG 때문에 HTTP Blob decode가 실패했다. PNG와 재실행 동작을 수정한 뒤
두 모니터의 실제 Seelen WebView2에서 icon/image decode 통과를 확인했다. 입력,
재시작, 절전, DPI, 포트 충돌, 방화벽 및 설치 모드 검수가 남아 있으므로 M0 전체
통과나 M1 진행 가능으로 판정하지 않는다.
...
따라서 손상 PNG로 실패했던 HTTP Blob 항목은 수정본에서 **통과**다. 나머지 필수
실기기 매트릭스가 남아 있으므로 M1 판정은 계속 **보류**다.
...
| 요구사항 | 통과 기준 | 현재 결과 | 증거 |
|---|---|---|---|
| 최소 Desktop | 화면 뒤 Desktop preset으로 표시 | 통과 | 수정본 두 모니터 표시 |
| 최소 Popup | trigger 뒤 input focus, 문자·한글·Enter·Esc 수신 | 대기 | |
| Companion 싱글턴 | 반복 재연결 뒤 Companion 프로세스 1개 | 기본 1개·20초 안정 통과, 반복 재연결 대기 | PID `23232` |
...
| 다중 모니터 | 모니터별 Desktop replica, Popup 위치·입력 정상 | Desktop replica 통과·Popup 대기 | 두 모니터에 각 1개 |
