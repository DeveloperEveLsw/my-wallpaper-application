...
- Windows Seelen 실기기 검수: 1차 Blob 실패, 수정 후 재검증 대기
- M1 판정: **보류**
- 기준일: 2026-07-24
...
손상된 시험 PNG 때문에 HTTP Blob decode가 실패했다. PNG와 재실행 동작을 수정했지만
실제 Seelen 재검수 전이므로 M0 통과나 M1 진행 가능으로 판정하지 않는다.
...
이 실패는 M0 실패로 기록한다. 수정본의 실제 Seelen Blob decode가 통과할 때까지
HTTP Blob 행은 미통과이며 M1 판정은 **보류**다.
...
| 요구사항 | 통과 기준 | 현재 결과 | 증거 |
|---|---|---|---|
| 최소 Desktop | 화면 뒤 Desktop preset으로 표시 | 1차 표시 확인·수정본 대기 | 2026-07-24 GUI 등록 |
...
| nonce/hello | 256-bit nonce, helloAck, replay 거부 | 1차 hello 확인·수정본 대기 | 2026-07-24 화면 |
| 메모리 토큰 | 파일·설정·localStorage·로그에 토큰 없음 | 코드 통과·실기기 대기 | |
| HTTP Blob | icon/image status·URL·type·length·PNG·decode 통과 | 1차 손상 PNG 실패·수정본 대기 | decode 오류 |
| 실제 Origin | 표시값이 exact allowlist와 일치 | 1차 exact 확인·수정본 대기 | `http://tauri.localhost` |
| 실제 Host | 선택 포트의 exact IPv4 Host만 수락 | 1차 확인·수정본 대기 | `127.0.0.1:43127` |
