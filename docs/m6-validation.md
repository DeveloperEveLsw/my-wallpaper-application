...
- 구현 상태: 완료
- 자동 검증: 통과
- 실제 Wallpaper Engine 통합 검수: 기존 호스트 완료, watchdog 입력 라우터 재검증 필요
...
경로를 확인했다. 전역 hook을 제거하고 기존의 비-suspend watchdog에 단일 60 Hz 포인터
라우터를 배치했다. 아래 자동 검증은 갱신했으며 pause·resume 실기기 항목은 재검증이 필요하다.
...
suspend되지 않는 watchdog 입력 라우터는 단일 60 Hz 루프에서 포인터 위치를 확인하고,
전달된 parent rect 안에 있을 때만 이 상태를 유지한다. 다른 모니터로 이동하면 WorkerW를
다시 disabled·입력 투과 상태로 만든다. 월페이퍼 프로세스에는 시스템 전역 저수준 마우스
hook을 설치하지 않으므로 Wallpaper Engine pause가 시스템 입력 전달을 막지 않는다.
WorkerW window region은 설정하지 않아 같은 WorkerW의 다른 모니터 wallpaper child가 계속
렌더링된다. 정상 dispose와 별도 watchdog은 이 변경을 역순으로 복원한다.
...
- M6 기능 범위는 주 모니터 하나지만, 보조 모니터의 Wallpaper Engine 렌더를 방해하지 않는
  것은 필수 비회귀 조건이다.
- watchdog 포인터 입력 전환, Wallpaper Engine pause 중 시스템 마우스 응답, 보조 모니터
  wallpaper 렌더 보존은 Windows 실기기에서 재검증해야 한다.
