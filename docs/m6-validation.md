...
- 구현 상태: 완료
- 자동 검증: 통과
- 실제 Wallpaper Engine 통합 검수: 기존 호스트 완료, 교차 프로세스 hang 수정 재검증 필요
...
비활성·입력 투과 상태로 만든 WorkerW는 초기 parent attach에서 한 번만 보정한다. 공유
WorkerW 자체의 window region은 제한하지 않아 보조 모니터 wallpaper를 함께 자르지 않는다.
일반 reload, Explorer 재시작 뒤의 복구, 앱·Wallpaper Engine 강제 종료 뒤 입력 복원과 원본
Desktop 아이콘 숨김도 각각 검증했다.

이후 월페이퍼 프로세스 안의 시스템 전역 저수준 마우스 hook이 Wallpaper Engine pause로
프로세스와 함께 suspend되면서 Windows 입력 전달을 지연시키는 회귀가 확인됐다. Wallpaper
Engine의 최대화·전체화면 동작을 `계속 실행`으로 바꾸면 지연이 사라지는 A/B 결과로 suspend
경로를 확인했다. 전역 hook을 제거한 뒤 비-suspend watchdog에 포인터 라우터를 배치했으나,
배포 직후 Windows WER에 `AppHangXProcB1`이 반복 기록됐다. `wallpaper64.exe`가 suspend된
`Wallpaper.App.exe`를 기다린 교차 프로세스 hang으로, watchdog이 Wallpaper Engine 소유
WorkerW를 계속 변경한 것이 첫 번째 원인이었다. 이를 제거한 뒤에도 22:57:05와 22:58:53에
같은 대기가 재발해, 앱 내부 500ms poll의 WorkerW 변경, 별도 overlay WPF child 및
WebView2 내부 HWND의 반복 재부모화가 남은 교차 프로세스 창 경합으로 확인됐다. 현재 구현은
Wallpaper Engine parent 아래에 WPF HWND 하나만 유지하고, 앱 소유 WebView2 composition
HWND는 최초 attach 또는 renderer 복구 완료 이벤트에서만 WPF HWND로 되돌린다. WorkerW
입력도 초기 attach에서만 설정하며 watchdog은 종료 관찰·최종 복원만 수행한다. WebView2는
user-data folder를 다른 앱 프로세스와 공유하지 않으며
브라우저·렌더러 실패 또는 10초 준비 timeout에는 control을 최대 5회 재생성한다. 아래 자동
검증은 갱신했으며 pause·resume, 좌·우클릭, 다중 모니터 입력 범위와 WebView 복구는 실기기
재검증이 필요하다.
...
| 입력 범위 격리 | 설계 변경·실기기 재검증 필요 | suspend 경합을 막기 위해 포인터 기반 WorkerW 반복 전환을 제거하고 초기 attach에서 한 번만 활성화 |
...
| WebView 프로세스 복구 | 구현 완료·실기기 재검증 필요 | 브라우저·주 renderer 실패와 `ready` timeout 뒤 WebView2 control 재생성 |
...
이 작업은 초기 parent attach에서 한 번만 실행한다. 이후 host poll은 parent 연결과 표시
상태만 읽고 WorkerW, WPF child 또는 WebView2 내부 HWND를 변경하지 않는다. 앱 소유
WebView2 composition HWND의 parent 보정은 최초 attach와 renderer 복구 완료 이벤트에서만
수행한다. 시스템 전역 저수준 마우스 hook을 설치하지 않으므로 Wallpaper Engine pause가
시스템 입력 전달을 막지 않는다. 앱이 suspend되면 host poll도 멈추고 watchdog은 HWND를
변경하지 않는다. WorkerW window region은 설정하지 않아 같은 WorkerW의 다른 모니터
wallpaper child가 계속 렌더링된다. 정상 dispose와 별도 watchdog의 종료 경로는 다른 활성
wallpaper child가 없을 때만 변경 전 입력 상태를 복원한다.
...
- 전체 빌드 경고 0, 오류 0
- Core 테스트 42개 통과
- Windows Infrastructure 테스트 42개 통과
- Host 테스트 30개 통과
- 합계 114개 테스트 통과
...
- 동일 parent의 창 배치·입력 초기화 비반복과 정상 dispose 복원 호출
...
시점의 앱 PID를 관찰하되 앱이 살아 있는 동안 parent/WorkerW HWND를 읽거나 변경하지 않는다.
엔진이 종료되면 정확한 앱 PID만 종료한 뒤 시작 시 검증한 WorkerW의 입력 상태를 복원한다.
앱 강제 종료와 엔진 강제 종료를 각각 시험했을 때 WorkerW는 disabled,
...
- M6 기능 범위는 주 모니터 하나지만, 보조 모니터의 Wallpaper Engine 렌더를 방해하지 않는
  것은 필수 비회귀 조건이다.
- Wallpaper Engine pause 중 시스템 마우스 응답, `AppHangXProcB1` 비재발, WebView2
  검정 화면 자동 복구와 보조 모니터 wallpaper 렌더 보존은 Windows 실기기에서
  재검증해야 한다.
