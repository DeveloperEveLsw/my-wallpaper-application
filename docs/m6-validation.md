...
보정했다. 일반 reload, Explorer 재시작 뒤의 복구, 앱·Wallpaper Engine 강제 종료 뒤
...
| 입력 범위 격리 | 통과 | WorkerW region을 주 모니터 parent rect로 제한하고 보조 모니터 좌표는 WPF hit-test에서 제외 |
...
WorkerW가 가상 Desktop 전체를 덮는 환경에서는 `SetWindowRgn`을 전달된 parent의 화면
rect로 제한해 다른 모니터의 입력을 가로채지 않는다. 정상 dispose와 별도 watchdog은
이 변경을 역순으로 복원한다.
...
- WorkerW region은 가상 Desktop client 기준 `(1600, 859, 3520, 1939)`, 화면 기준
  `(0, 0, 1920, 1080)`으로 주 모니터에만 적용됐다.
...
- M6는 주 모니터 하나만 검증했다. 보조 모니터와 모니터별 독립 상태는 MVP 이후다.
- 보조 모니터 렌더링은 범위 밖이지만 주 모니터용 입력 region이 보조 모니터 클릭을
  가로채지 않는 것은 검증했다.
