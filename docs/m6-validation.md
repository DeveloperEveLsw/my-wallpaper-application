...
WebView2 내부 HWND 수동 재부모화가 남은 교차 프로세스 창 경합으로 확인됐다. 현재 구현은
Wallpaper Engine parent 아래에 WPF HWND 하나만 유지하고, WebView2 내부 HWND를 건드리지
않으며, WorkerW 입력은 초기 attach에서만 설정한다. watchdog은 종료 관찰·최종 복원만
수행한다. WebView2는 user-data folder를 다른 앱 프로세스와 공유하지 않으며
...
상태만 읽고 WorkerW, WPF child 또는 WebView2 내부 HWND를 변경하지 않는다. 시스템 전역
저수준 마우스 hook을 설치하지 않으므로 Wallpaper Engine pause가 시스템 입력 전달을 막지
않는다. 앱이 suspend되면 host poll도 멈추고 watchdog은 HWND를 변경하지 않는다. WorkerW
window region은 설정하지 않아 같은 WorkerW의 다른 모니터 wallpaper child가 계속
렌더링된다. 정상 dispose와 별도 watchdog의 종료 경로는 다른 활성 wallpaper child가 없을
때만 변경 전 입력 상태를 복원한다.
