...
WorkerW를 계속 변경한 것이 원인이었다. 따라서 포인터 라우팅은 다시 월페이퍼 프로세스
내부의 직렬 poll로 제한하고 watchdog은 종료 관찰·최종 복원만 수행한다. WebView2는
user-data folder를 다른 앱 프로세스와 공유하지 않으며 브라우저·렌더러 실패 또는 10초 준비
timeout에는 control을 최대 5회 재생성한다. 아래 자동 검증은 갱신했으며 pause·resume 및
WebView 복구는 실기기 재검증이 필요하다.
...
| 입력 범위 격리 | 자동 검증 통과·실기기 재검증 필요 | 실행 중인 앱의 host poll에서 포인터가 주 모니터 parent rect 안에 있을 때만 공유 WorkerW 입력 활성화 |
...
월페이퍼 프로세스 내부의 host poll은 전달된 parent rect 안에 포인터가 있을 때만 이 상태를
유지한다. 다른 모니터로 이동하면 WorkerW를 다시 disabled·입력 투과 상태로 만든다. 시스템
전역 저수준 마우스 hook을 설치하지 않으므로 Wallpaper Engine pause가 시스템 입력 전달을
막지 않는다. 앱이 suspend되면 host poll도 멈추고 watchdog은 HWND를 변경하지 않는다.
WorkerW window region은 설정하지 않아 같은 WorkerW의 다른 모니터 wallpaper child가 계속
렌더링된다. 정상 dispose와 별도 watchdog의 종료 경로는 이 변경을 역순으로 복원한다.
