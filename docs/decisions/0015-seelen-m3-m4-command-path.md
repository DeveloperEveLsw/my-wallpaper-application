...
- 파일 drag와 Dock 폴더 순서 drag는 서로 다른 상태로 관리한다.
...
실제 Seelen 렌더링, 전역 입력 투과, 네이티브 Explorer 실행, Windows 휴지통 UI,
drag preview와 Glass 배치는 Windows Seelen 실기기 검수로 분리한다. 세부 항목은
