...
`SetWindowRgn`으로 자르지 않는다. WorkerW 입력 초기화는 parent attach 시 한 번만 수행하고
원래 enabled/style 값을 보관한다. 주기 host poll은 parent 연결·표시 상태만 읽으며 동일
parent의 `SetParent`, `EnableWindow`, style 또는 z-order를 반복 변경하지 않는다.
다른 스레드 소유 창의 위치 복원에는 `SWP_ASYNCWINDOWPOS`를 사용한다. suspend될 수 있는
월페이퍼 프로세스에는 시스템 전역 저수준 마우스 hook을 설치하지 않으며, 비-suspend
watchdog도 앱이 살아 있는 동안 Wallpaper Engine 소유 HWND를 변경하지 않는다. 정상 종료
시에는 다른 활성 wallpaper child가 없을 때만 보관한 WorkerW 상태를 복원한다.

Wallpaper 화면은 Wallpaper Engine parent 아래의 WPF HWND 하나로 구성한다.
`WebView2CompositionControl`이 같은 WPF visual tree 안에서 Web 배경과 WPF UI를 합성하므로,
별도 투명 오버레이 HWND를 만들지 않는다. 단, WPF HWND의 parent가 바뀔 때 Wallpaper Engine
parent 아래에 분리되는 앱 소유 `Chrome_WidgetWin_0`은 WPF HWND로 한 번 되돌려 WPF UI의
z-order와 입력 hit-test를 보존한다. WebView2가 renderer를 재생성한 경우에도 완료 이벤트에서
같은 보정을 한 번 수행하며, 주기 poll에서는 이 HWND를 검색하거나 변경하지 않는다.
...
- 동일 parent의 WorkerW 및 앱 HWND를 주기적으로 변경하지 않아 suspend 전환과
  `AppHangXProcB1` 교차 프로세스 대기 가능성을 제거한다.
- Wallpaper Engine parent 아래에는 앱 소유 WPF HWND가 하나만 존재한다.
- 보조 모니터의 wallpaper child는 공유 WorkerW 영역에 의해 시각적으로 잘리지 않는다.
- 공유 WorkerW의 다중 모니터 입력 범위는 실기기 재검증 후 별도 capability로 확정한다.
