...
`SetWindowRgn`으로 자르지 않는다. 별도 저수준 마우스 입력 라우터가 포인터 위치를 관찰해
전달된 parent의 모니터 rect 안에서만 WorkerW 입력을 활성화한다. host dispose 시 WorkerW를
다시 비활성화하고 투과 style, 무영역 상태와 Desktop view의 z-order를 복원한다. 주기
polling은 style, parent, rect 또는 WorkerW 입력 상태가 실제로 달라졌을 때만
`SetWindowPos`를 실행해 메뉴와 drag 도중의 native 입력 상태를 흔들지 않는다.
