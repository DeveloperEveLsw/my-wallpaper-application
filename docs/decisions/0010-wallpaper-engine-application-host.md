...
`SetWindowRgn`으로 자르지 않는다. Wallpaper Engine pause에 suspend되지 않는 watchdog의
단일 입력 라우터가 포인터 위치를 60 Hz로 관찰해 전달된 parent의 모니터 rect 안에서만
WorkerW 입력을 활성화한다. suspend될 수 있는 월페이퍼 프로세스에는 시스템 전역 저수준
마우스 hook을 설치하지 않는다. host dispose 시 WorkerW를 다시 비활성화하고 투과 style,
무영역 상태와 Desktop view의 z-order를 복원한다. 주기 polling은 style, parent, rect 또는
WorkerW 입력 상태가 실제로 달라졌을 때만 `SetWindowPos`를 실행해 메뉴와 drag 도중의
native 입력 상태를 흔들지 않는다.
...
앱·엔진 PID와 검증된 parent/WorkerW HWND를 관찰하면서 포인터 입력 범위를 유지한다.
앱 또는 엔진이 먼저 종료되면 새 wallpaper child가 WorkerW를 이미 사용 중인지 확인하고,
그렇지 않을 때 입력 상태를 복원한다. 엔진 종료 시에는 앱 PID도 종료한다. 정상 종료와
reload 스크립트는 watchdog을 포함해 정확히 같은 배포 실행 파일 경로의 프로세스만 종료한다.
